...
import { submitDemoRequest, trackDemoConversion, DemoFormData } from '@/lib/hubspot';
...
export default function DemoForm({ variant = 'light', onSuccess }: DemoFormProps) {
    ...
    return (
      <div
        className={`p-8 rounded-2xl text-center ${
          isDark ? 'bg-white/10' : 'bg-emerald-50 border border-emerald-200'
        }`}
      >
      ...
      <div>
        ...
        <div className="relative">
          <select
            ...
            value={formData.role}
            onChange={handleChange}
            className={`${selectClasses} ${errors.role ? 'border-red-500' : ''} ${
              !formData.role ? (isDark ? 'text-white/50' : 'text-slate-400') : ''
            }`}
          >
          ...
          <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
            <svg
              className={`w-5 h-5 ${isDark ? 'text-white/50' : 'text-slate-400'}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
      ...
      <div>
        ...
        <div className="relative">
          <select
            ...
            onChange={handleChange}
            className={`${selectClasses} ${errors.studentCount ? 'border-red-500' : ''} ${
              !formData.studentCount ? (isDark ? 'text-white/50' : 'text-slate-400') : ''
            }`}
          >
        ...
        <div className="relative">
          <select
            ...
            className={`${selectClasses} ${
              !formData.howDidYouHear ? (isDark ? 'text-white/50' : 'text-slate-400') : ''
            }`}
          >
